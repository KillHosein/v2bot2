...
# --- Logging ---
LOG_LEVEL = os.getenv("LOG_LEVEL", "WARNING").upper()  # Changed default from INFO to WARNING
logging.basicConfig(
    format="%(asctime)s - %(name)s - %(levelname)s - %(message)s",
    level=getattr(logging, LOG_LEVEL, logging.WARNING),
    handlers=[
        logging.StreamHandler()  # Use only stream handler to avoid file bloat
    ]
)
logger = logging.getLogger("bot")
# Reduce logging from libraries
